...
async def stream_user_events(user_id: str, token: str = Query(min_length=16), db=Depends(get_db)) -> StreamingResponse:
    del db
    return await _stream_authenticated_user_channel(user_id=user_id, token=token)
...
async def _stream_authenticated_user_channel(*, user_id: str, token: str) -> StreamingResponse:
