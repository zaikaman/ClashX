...
class TradingService:
    ...
    async def get_account_snapshot(self, db: Session, wallet_address: str) -> dict[str, Any]:
        user = self._upsert_user(db, wallet_address)
        authorization = self.auth_service.get_authorization_by_wallet(db, wallet_address)
        ...
        recent_activity = self._list_recent_activity(db, user_id=user["id"] if isinstance(user, dict) else user.id)
        ...
        win_rate = None
        if winning_fills:
            profitable = sum(1 for item in winning_fills if item["pnl"] > 0)
            win_rate = profitable / len(winning_fills)
    ...
    async def list_positions(self, db: Session, wallet_address: str) -> list[dict[str, Any]]:
        snapshot = await self.get_account_snapshot(db, wallet_address)
        return snapshot["positions"]

    async def list_orders(self, db: Session, wallet_address: str) -> list[dict[str, Any]]:
        snapshot = await self.get_account_snapshot(db, wallet_address)
    ...
    async def place_order(
        ...
        *,
        ...
        credentials = self.auth_service.get_trading_credentials(db, wallet_address)
        ...
        self._record_audit_event(
            db,
            user_id=user_id,
            action="trading.order.submitted",
            payload={
                "symbol": normalized_symbol,
                "side": side,
    ...
    async def cancel_order(
        ...
        *,
        ...
        self._record_audit_event(
            db,
            user_id=user_id,
            action="trading.order.cancelled",
            payload={"symbol": normalized_symbol, "order_id": order_id, "request_id": response["request_id"]},
        )
        snapshot = await self.get_account_snapshot(db, wallet_address)
        await self._publish_snapshot(
            user_id=user_id,
            event="trading.order.cancelled",
            payload={"order_id": order_id, "symbol": normalized_symbol, "request_id": response["request_id"]},
            snapshot=snapshot,
        )
        return {
            "status": response["status"],
    ...
    async def _get_market(self, symbol: str) -> dict[str, Any]:
        ...
        market = next(
            (
                ...
                or self._normalize_symbol(item.get("display_symbol")) == normalized_symbol
            ),
            None,
        )
        if market is None:
    ...
    async def _get_reference_price(self, symbol: str) -> float:
        ...
        return float(market.get("mark_price", 0) or 0)
    ...
    def _serialize_position(self, position: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "updated_at": position.get("updated_at") or position.get("created_at"),
        }
    ...
    def _serialize_order(self, order: dict[str, Any], market_lookup: dict[str, dict[str, Any]]) -> dict[str, Any]:
        ...
        return {
            "id": str(order.get("order_id") or order.get("client_order_id") or uuid.uuid4()),
            ...
            "notional_usd": quantity * reference_price if reference_price else 0.0,
            "limit_price": float(limit_price or 0) if limit_price is not None else None,
    ...
    def _serialize_fill(self, fill: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            "id": str(fill.get("history_id") or uuid.uuid4()),
            "symbol": symbol,
            "display_symbol": f"{symbol}-PERP",
            "side": side,
            ...
            "pnl": float(fill.get("pnl", 0) or 0),
            "liquidity": "maker" if bool(fill.get("is_maker", False)) else "taker",
            "filled_at": fill.get("created_at"),
        }

    def _list_recent_activity(self, db: Session, *, user_id: str) -> list[dict[str, Any]]:
        if self.settings.use_supabase_api:
            ...
            return [self._serialize_activity(event) for event in events]
    ...
    def _serialize_activity(self, event: AuditEvent | dict[str, Any]) -> dict[str, Any]:
    ...
    def _upsert_user(self, db: Session, wallet_address: str) -> User | dict[str, Any]:
        if self.settings.use_supabase_api:
            if self.supabase is None:
    ...
    def _record_audit_event(self, db: Session, *, user_id: str, action: str, payload: dict[str, Any]) -> None:
        ...
        db.add(AuditEvent(user_id=user_id, action=action, payload=payload, created_at=timestamp))
        db.commit()
