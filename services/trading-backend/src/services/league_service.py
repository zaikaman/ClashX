from __future__ import annotations

from datetime import UTC, datetime
from types import SimpleNamespace

from sqlalchemy import Select, desc, func, select
from sqlalchemy.orm import Session

from src.core.settings import get_settings
from src.models.leaderboard_snapshot import LeaderboardSnapshot
from src.models.league import League
from src.models.league_participant import LeagueParticipant
from src.models.user import User
from src.services.supabase_rest import SupabaseRestClient


class LeagueService:
    def __init__(self, supabase: SupabaseRestClient | None = None) -> None:
        self.settings = get_settings()
        self.supabase = supabase or (SupabaseRestClient() if self.settings.use_supabase_api else None)

    def list_leagues(self, db: Session, status: str | None = None) -> list[dict]:
        if self.settings.use_supabase_api:
            filters = {"status": status} if status else None
            leagues = self.supabase.select("leagues", filters=filters, order="start_at.desc")
            registrations = self.supabase.select("league_participants", filters={"is_active": True})
            registration_counts: dict[str, int] = {}
            for registration in registrations:
                league_id = registration["league_id"]
                registration_counts[league_id] = registration_counts.get(league_id, 0) + 1

            return [
                {
                    "id": league["id"],
                    "name": league["name"],
                    "description": league["description"],
                    "status": league["status"],
                    "market_scope": league["market_scope"],
                    "start_at": league["start_at"],
                    "end_at": league["end_at"],
                    "registration_count": registration_counts.get(league["id"], 0),
                }
                for league in leagues
            ]

        query: Select[tuple[League, int]] = (
            select(League, func.count(LeagueParticipant.id))
            .outerjoin(LeagueParticipant, LeagueParticipant.league_id == League.id)
            .group_by(League.id)
            .order_by(desc(League.start_at))
        )
        if status:
            query = query.where(League.status == status)

        leagues: list[dict] = []
        for league, registration_count in db.execute(query).all():
            leagues.append(
                {
                    "id": league.id,
                    "name": league.name,
                    "description": league.description,
                    "status": league.status,
                    "market_scope": league.market_scope,
                    "start_at": league.start_at,
                    "end_at": league.end_at,
                    "registration_count": registration_count,
                }
            )
        return leagues

    def register_bot(
        self,
        db: Session,
        *,
        league_id: str,
        wallet_address: str,
        display_name: str | None = None,
    ) -> dict:
        if self.settings.use_supabase_api:
            league = self.supabase.maybe_one("leagues", filters={"id": league_id})
            if league is None:
                raise ValueError("Competition not found")
            if league["status"] != "live":
                raise ValueError("Only live competitions accept registrations")

            user = self.supabase.maybe_one("users", filters={"wallet_address": wallet_address})
            if user is None:
                user = {
                    "id": self._uuid(),
                    "wallet_address": wallet_address,
                    "display_name": display_name or wallet_address[:8],
                    "auth_provider": "privy",
                    "created_at": datetime.now(tz=UTC).isoformat(),
                }
                self.supabase.insert("users", user)
            elif display_name and user.get("display_name") != display_name:
                user = self.supabase.update("users", {"display_name": display_name}, filters={"id": user["id"]})[0]

            memberships = self.supabase.select(
                "league_participants",
                filters={"user_id": user["id"], "is_active": True},
            )
            if memberships:
                league_ids = [membership["league_id"] for membership in memberships]
                live_leagues = self.supabase.select("leagues", filters={"id": ("in", league_ids), "status": "live"})
                for live in live_leagues:
                    if live["id"] == league_id:
                        return {"league_id": league_id, "already_registered": True}
                    raise ValueError("Wallet already has a bot registered in another live competition")

            self.supabase.insert(
                "league_participants",
                {
                    "id": self._uuid(),
                    "league_id": league_id,
                    "user_id": user["id"],
                    "joined_at": datetime.now(tz=UTC).isoformat(),
                    "is_active": True,
                },
            )
            return {
                "league_id": league_id,
                "user_id": user["id"],
                "display_name": user.get("display_name"),
                "already_registered": False,
            }

        league = db.get(League, league_id)
        if league is None:
            raise ValueError("Competition not found")
        if league.status != "live":
            raise ValueError("Only live competitions accept registrations")

        active_membership = db.scalar(
            select(LeagueParticipant)
            .join(League, League.id == LeagueParticipant.league_id)
            .join(User, User.id == LeagueParticipant.user_id)
            .where(
                LeagueParticipant.is_active.is_(True),
                League.status == "live",
                User.wallet_address == wallet_address,
            )
            .limit(1)
        )
        if active_membership:
            if active_membership.league_id == league_id:
                return {"league_id": league_id, "already_registered": True}
            raise ValueError("Wallet already has a bot registered in another live competition")

        user = db.scalar(select(User).where(User.wallet_address == wallet_address).limit(1))
        if user is None:
            user = User(wallet_address=wallet_address, display_name=display_name or wallet_address[:8])
            db.add(user)
            db.flush()
        elif display_name:
            user.display_name = display_name

        registration = LeagueParticipant(league_id=league_id, user_id=user.id)
        db.add(registration)
        db.commit()
        return {
            "league_id": league_id,
            "user_id": user.id,
            "display_name": user.display_name,
            "already_registered": False,
        }

    def join_league(
        self,
        db: Session,
        *,
        league_id: str,
        wallet_address: str,
        display_name: str | None = None,
    ) -> dict:
        return self.register_bot(
            db,
            league_id=league_id,
            wallet_address=wallet_address,
            display_name=display_name,
        )

    def get_league(self, db: Session, league_id: str) -> League | None:
        if self.settings.use_supabase_api:
            row = self.supabase.maybe_one("leagues", filters={"id": league_id})
            return SimpleNamespace(**row) if row else None
        return db.get(League, league_id)

    def get_live_leagues(self, db: Session) -> list[League]:
        if self.settings.use_supabase_api:
            return [SimpleNamespace(**row) for row in self.supabase.select("leagues", filters={"status": "live"})]
        return list(db.scalars(select(League).where(League.status == "live")).all())

    def get_leaderboard(self, db: Session, league_id: str, limit: int = 100) -> list[dict]:
        if self.settings.use_supabase_api:
            rows = self.supabase.select("leaderboard_snapshots", filters={"league_id": league_id}, order="captured_at.desc", limit=limit)
            if not rows:
                return []
            latest = rows[0]["captured_at"]
            rows = [row for row in rows if row["captured_at"] == latest][:limit]
            user_ids = [row["user_id"] for row in rows]
            users = {
                row["id"]: row
                for row in self.supabase.select("users", filters={"id": ("in", user_ids)})
            } if user_ids else {}
            rows.sort(key=lambda row: row["rank"])
            return [
                {
                    "user_id": row["user_id"],
                    "display_name": users.get(row["user_id"], {}).get("display_name") or users.get(row["user_id"], {}).get("wallet_address", "")[:8],
                    "wallet_address": users.get(row["user_id"], {}).get("wallet_address", ""),
                    "rank": row["rank"],
                    "unrealized_pnl": row["unrealized_pnl"],
                    "realized_pnl": row["realized_pnl"],
                    "win_streak": row["win_streak"],
                    "captured_at": row["captured_at"],
                }
                for row in rows
            ]

        latest = db.scalar(
            select(LeaderboardSnapshot.captured_at)
            .where(LeaderboardSnapshot.league_id == league_id)
            .order_by(desc(LeaderboardSnapshot.captured_at))
            .limit(1)
        )
        if latest is None:
            return []

        rows = db.execute(
            select(LeaderboardSnapshot, User)
            .join(User, User.id == LeaderboardSnapshot.user_id)
            .where(
                LeaderboardSnapshot.league_id == league_id,
                LeaderboardSnapshot.captured_at == latest,
            )
            .order_by(LeaderboardSnapshot.rank.asc())
            .limit(limit)
        ).all()

        leaderboard: list[dict] = []
        for snapshot, user in rows:
            leaderboard.append(
                {
                    "user_id": user.id,
                    "display_name": user.display_name or user.wallet_address[:8],
                    "wallet_address": user.wallet_address,
                    "rank": snapshot.rank,
                    "unrealized_pnl": snapshot.unrealized_pnl,
                    "realized_pnl": snapshot.realized_pnl,
                    "win_streak": snapshot.win_streak,
                    "captured_at": snapshot.captured_at,
                }
            )
        return leaderboard

    @staticmethod
    def _uuid() -> str:
        import uuid

        return str(uuid.uuid4())
