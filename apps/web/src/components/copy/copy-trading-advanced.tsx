    ...
    <div className="grid gap-6 md:gap-8">
        ...
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div className="grid gap-2">
            ...
            <p className="max-w-3xl text-sm leading-7 text-neutral-400">
              Keep basket tooling available for advanced allocation work without turning the main copy page into a portfolio builder first.
            </p>
          </div>
          <button
            type="button"
          ...
          >
            {showComposer ? "Close composer" : "Create basket"}
          </button>
        </div>
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          {basketSummaries.length === 0 ? (
            <article className="rounded-[1.5rem] border border-dashed border-[rgba(255,255,255,0.08)] bg-[#101214] px-4 py-5 text-sm leading-6 text-neutral-400 md:col-span-2 xl:col-span-4">
              No baskets yet. Use this area only when you want a multi-trader allocation layer on top of direct copy follows.
            </article>
          ) : (
            basketSummaries.map((basket) => (
              <article key={basket.id || basket.name} className="grid gap-1 rounded-[1.35rem] border border-[rgba(255,255,255,0.06)] bg-[#101214] px-4 py-4">
                ...
                <span className="text-sm text-neutral-400">{basket.member_count} members · {formatUsd(basket.target_notional_usd)} target</span>
      ...
      {showComposer ? (
        <PortfolioBasketComposer
          ...
          editingLabel={editingPortfolioId ? "Editing basket" : "Create basket"}
